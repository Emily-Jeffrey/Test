...
function handleMouseDown(e) {
    ...
    if (yearKnob.startDrag(x, y) || lengthKnob.startDrag(x, y) || volumeKnob.startDrag(x, y)) {
...
function handleMouseMove(e) {
    if (!yearKnob.isDragging && !lengthKnob.isDragging && !volumeKnob.isDragging) return;
    ...
    volumeKnob.updateFromMouse(x, y);
    drawKnobs();
}
...
function updateKnobImages() {
    ...
    const knob3Img = document.getElementById('knob3-img');
    ...
    if (knob3Img) {
        const degrees3 = volumeKnob.angle * (180 / Math.PI);
        knob3Img.style.transform = `rotate(${degrees3}deg)`;
    }
}
...
function updateVolumeLevel(value) {
    if (typeof setSpeechVolume === 'function') {
...
function initKnobs() {
    ...
    volumeKnob = new RotaryKnob(
        ...
        (vol) => {
            ...
            updateVolumeLevel(normalized);
        }
    );
    
    yearKnob.setValue(2025);
    lengthKnob.setValue(1);
    volumeKnob.setValue(60); // default 0.6 volume
